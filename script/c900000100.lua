...
function s.initial_effect(c)
    ...
    s.counter_list={ [0x101]={id=id,name=aux.Stringid(id,4)} }

    -- ATK/DEF par compteur
...
function s.atkval(e,c)
    return math.min(c:GetCounter(0x101),5)*200
end
...
function s.addcounter(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    if c:GetCounter(0x101)<5 then
        c:AddCounter(0x101,1)
    end
end

-- Coût pour annuler un effet
function s.discost(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return e:GetHandler():GetCounter(0x101)>0 end
    e:GetHandler():RemoveCounter(tp,0x101,1,REASON_COST)
end

-- Nouveau filtre pour annulation (compatible 2024–2025)
function s.negfilter(c)
    return (c:IsMonster() and c:IsNegatableMonster())
        or (c:IsSpellTrap() and c:IsNegatableSpellTrap())
end
